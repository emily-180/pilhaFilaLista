...
//------------------------
int verificaFila(tfila f){
	if(isEmpty(f) == 1)
		printf("Fila vazia!");
	else{
		tno* atual = f.inicio;
		while (atual != NULL && atual->prox != NULL) {
			if(atual->dado.valor > atual->prox->dado.valor)
	        	return 0;	
			atual = atual->prox;
		}	
	}
	...
	do{
		...
		switch(op){
			...
			case 2: if(!isEmpty(f1)) {
				...
				break;
			case 3:
				if(verificaFila(f1)==1)
					printf("Fila esta em ordem cresente!");
				else 
					printf("Fila nao esta em ordem cresente!");
			break;	
